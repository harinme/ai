...
plot(month,late1,main='월별 지각 학생 수',type='b',col='red',xlab='월',ylab = '지각인원수',ylim = c(1,15))
...
install.packages('mlbench') # 패키지를 다운받기
library(mlbench) # 다운받은 자료를 사용할 수 있게 열어주기
data('BostonHousing') # 라이브러리 안에 있는 여러 데이터셋 중 한개를 사용하기
...
head(grp)
class(grp)
grp <- factor(grp) # 문자형 자료를 범주형 자료(factor)형 변환
class(grp)
head(grp)
grp<- factor(grp,levels = c('H','M','L')) # levels로 factor의 순서를 사용자가 정의 가능
head(grp)
...
# 4 차트를 그려서 분석(실제 값으로 해석하기)
par(mfrow=c(2,3))
for(i in 1:5){
  hist(myds[,i],main=colnames(myds)[i],col='yellow',ylim = c(0,300))
}
...
# 그룹 별 관측값 분포 확인
par(mfrow=c(1,1))
# 구하려는 값(y축)~그룹화할 변수(x축), data=데이터 셋 
...
head(grp)
# levels를 숫자로 바꾸고 싶다 h-1 m-2 l-3
point<- as.integer(myds$grp)
unique(point)
head(point)
color <- c('#ff0000','#00ff00','#0000ff')
pairs(myds[,1:5],main='보스턴 주택가격 영향 요소',col=color,pch=point)
