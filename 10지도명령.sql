-- 569 P 14장 지리 정보 시스템
-- mysql 5.0 이후는 geometry 자료형 지원함.

DROP DATABASE IF EXISTS GisDB;
CREATE DATABASE GisDB;

USE GisDB;
drop table streamtbl;
...
SELECT * FROM BuildingTbl
UNION ALL
SELECT * FROM StreamTbl WHERE ST_Length(Stream) > 140 ; -- st_length() 길이 구하는 함수
...
-- st-intersection 두 도형을 교차한 결과 좌표 집합을 구한다.
